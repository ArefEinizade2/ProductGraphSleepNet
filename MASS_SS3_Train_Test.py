...
from MASS_SS3_DataGenerator import *
...
args = parser.parse_args(["-c", "./MASS_SS3.config", "-g", "0"])
...
shutil.copyfile(args.c, Path['Save']+"MASS_SS3_last.config")
...
for i in range(fold):
    ...
    if i==0:
        model.summary()
        plot_model(model, to_file=str(Path['Save'] + 'MASS_SS3_model.png'), show_shapes=True)
    ...
    if use_pretrained=='False': # if pretrain is selected, the model is not trained
        ...
        history = model.fit(
            ...
            callbacks=[keras.callbacks.ModelCheckpoint(Path['Save']+'MASS_SS3_Best_model_'+str(i)+'.h5', 
                                                       monitor='val_acc', 
                                                       verbose=2, 
                                                       save_best_only=True, 
        ...
        model.load_weights(Path['Save']+'MASS_SS3_Best_model_'+str(i)+'.h5')
    else:
        # Evaluate
        # Load weights of best performance
        model.load_weights(Path['pretrain_folder']+'MASS_SS3_Best_model_'+str(i)+'.h5')
...
# save learned graphs:
np.save(Path['Save'] + 'MASS_SS3_LearnedGraphsSpatial.npy', np.concatenate(predicts_learnedSpatialGraphs_All_folds, axis=0))
np.save(Path['Save'] + 'MASS_SS3_LearnedGraphsTempral.npy', np.concatenate(predicts_learnedTemporalGraphs_All_folds, axis=0))
...
np.save(Path['Save'] + 'MASS_SS3_Labels_All.npy', AllTrue)
...
PrintScore2(AllTrue, AllPred, all_scores, savePath=Path['Save']+'MASS_SS3_')

# Print confusion matrix and save
ConfusionMatrix(AllTrue, AllPred, classes=['W','N1','N2','N3','REM'], savePath=Path['Save']+'MASS_SS3_', title='Confusion matrix MASS-SS3')
...
plt.show()
plt.savefig(Path['Save'] + 'MASS_SS3_AUC_AUPRC_plot.eps')   
plt.savefig(Path['Save'] + 'MASS_SS3_AUC_AUPRC_plot.png')   
plt.savefig(Path['Save'] + 'MASS_SS3_AUC_AUPRC_plot.pdf')   
